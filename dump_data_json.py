...
def calc_level(limit_text, has_source=False):
    ...
    # Level 0 — подтверждённая свобода. ВАЖНО: "без ограничений" встречается как
    # подпункт ("дизель без ограничений") в тексте, где есть настоящие лимиты.
    # Если в тексте есть цифры с "л", "запрет", "лимит" — это не уровень 0.
    if ('без ограничений' in l or 'без лимит' in l or l == 'свободно'):
        if not re.search(r'\d\s*л|запрет|лимит|только в бак|канистр|четн|нечетн|по госномер|по номерам|по дням', l):
